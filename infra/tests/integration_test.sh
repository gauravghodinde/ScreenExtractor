...
# Integration test: POST /jobs then poll Postgres until status != queued
# Usage: bash infra/tests/integration_test.sh [timeout_seconds]
...
BODY='{"video_path":"s3://scene-uploads/video.mkv","start":1,"end":3,"query":"integration test"}'
...
while true; do
  ...
  if (( ELAPSED > TIMEOUT )); then
    ...
    # show last 20 lines of worker logs for debugging
  ...
  if [[ -z "$STATUS" ]]; then
    ...
    if [[ "$STATUS" != "queued" ]]; then
      echo "Job reached terminal status: $STATUS"
      # show worker logs around processing
      echo "--- worker logs (last 200 lines) ---"
      docker logs --tail 200 infra-worker-1 || true
      exit 0
    fi
